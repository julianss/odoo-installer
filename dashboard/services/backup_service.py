    ...
    try:
        ...
        if include_filestore:
            ...
            if os.path.exists(source_filestore):
                # Odoo stores filestore under <filestore_dir>/<database_name>/
                # When copying between environments, we must rename the DB subdirectory
                source_db_filestore = os.path.join(source_filestore, source_db)

                if os.path.exists(source_db_filestore):
                    # Copy source DB filestore to target under the target DB name
                    target_db_filestore = os.path.join(target_filestore, target_db)

                    if os.path.exists(target_db_filestore):
                        shutil.rmtree(target_db_filestore)

                    shutil.copytree(source_db_filestore, target_db_filestore)
                else:
                    # Fallback: copy entire filestore directory if DB subdirectory not found
