...
def create_databases_and_users(config):
    """Create PostgreSQL users for all environments.
    ...
    for env in environments:
        ...
        if stdout.strip() == "1":
            ...
            create_user_cmd = f"sudo -u postgres psql -c \"CREATE USER {db_user} WITH LOGIN PASSWORD '{db_pass}' CREATEDB;\""
...
def generate_nginx_config(config):
    ...
    primary_domain = domain_prod

    # Start building config with map and upstreams
    nginx_conf = f"""map $http_upgrade $connection_upgrade {{
  default upgrade;
  ''      close;
}}

upstream odoochat_test {{
    server {primary_domain}:{port_lp_test};
}}
upstream odoo_test {{
    server {primary_domain}:{port_http_test};
}}

upstream odoochat_staging {{
    server {primary_domain}:{port_lp_staging};
}}
upstream odoo_staging {{
    server {primary_domain}:{port_http_staging};
}}

upstream odoochat_prod {{
    server {primary_domain}:{port_lp_prod};
}}
upstream odoo_prod {{
    server {primary_domain}:{port_http_prod};
}}
...
"""
    ...
    environments = [
        ('prod', domain_prod, ssl_cert_prod, ssl_key_prod, 'odoo_prod', 'odoochat_prod', '50M'),
        ('staging', domain_staging, ssl_cert_staging, ssl_key_staging, 'odoo_staging', 'odoochat_staging', '1200M'),
        ('test', domain_test, ssl_cert_test, ssl_key_test, 'odoo_test', 'odoochat_test', '800M'),
    ]

    for env_name, domain, ssl_cert, ssl_key, upstream, upstream_chat, max_body_size in environments:
        if skip_ssl:
...
"""
            ...
            # HTTPS configuration with redirect and Let's Encrypt support
            nginx_conf += f"""server {{
    listen 80;
    listen [::]:80;
    server_name {domain};
...
def run_dry_run_installation(config):
    ...
    update_install_progress("Checking database user setup", 35)
    for env in ['test', 'staging', 'prod']:
        add_install_log(f"Would create database user for {env} with CREATEDB privilege")
    add_install_log("Note: Databases will be created by Odoo on first access")
...
def run_full_installation(config):
    ...
    update_install_progress("Creating database users", 45)
    add_install_log("Creating PostgreSQL database users...")
    success, message = create_databases_and_users(config)
    add_install_log(message)
    if not success:
        raise Exception(f"Database user creation failed: {message}")
            ...
            <form id="wizardForm">
                ...
                <div class="step" data-step="2">
                    ...
                    <div class="alert alert-warning">
                        <strong>Important:</strong> PostgreSQL must already be installed and running. The installer will create database users only. Odoo will create and initialize the databases itself on first access through its web interface.
                    </div>
